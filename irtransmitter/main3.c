 ...
 *
 */
static const rmt_item32_t turn_on_ac[] = {
    // E : dot
    {{{ 32767, 1, 32767, 0 }}}, // dot
...
{
    rmt_config_t config = RMT_DEFAULT_CONFIG_TX(IR_LED, RMT_TX_CHANNEL);
...
void app_main(void *ignore)
{
    ...
    while (1) {
        ESP_ERROR_CHECK(rmt_write_items(RMT_TX_CHANNEL, turn_on_ac, sizeof(turn_on_ac) / sizeof(turn_on_ac[0]), true));
